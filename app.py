...
def parse_gemini_response(text):
    ...
    try:
        # The model often wraps the JSON in ```json ... ```, so we strip that
        if text.strip().startswith("```json"):
            text = text.strip()[7:-3]
        return json.loads(text)
    except json.JSONDecodeError:
        st.warning("Failed to decode JSON from model response. Skipping this item.")
        # st.text_area("Faulty Response", text) # Uncomment for debugging
        return None
                ...
                with st.spinner("Athena is crafting behavioral dilemmas... This may take a few minutes."):
                    for i, row in df_competencies.iterrows():
                        ...
                        if len(pl1_indicators) < 2:
                            st.warning(f"Skipping '{competency_name}' as it has fewer than 2 indicators.")
                            continue
                        ...
                        for j in range(5):
                            ...
                            # Pad with empty strings if less than 4 indicators
                            ...
                            try:
                                ...
                                if parsed_data:
                                    ...
                                    # Ensure options are sorted PH, PL, NL, NH for consistent output
                                    ...
                                    st.warning(f"Could not parse response for '{competency_name}' SJT {j+1}. Trying again...")
