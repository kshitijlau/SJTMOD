...
if uploaded_file:
    try:
        ...
        if not required_columns.issubset(df_competencies_raw.columns):
            ...
            # --- DATA CLEANING STEP ---
            df_competencies = df_competencies_raw.dropna(subset=['Competency Name', 'Indicators (PL1)']).copy()
            df_competencies['Competency Name'] = df_competencies['Competency Name'].astype(str).str.strip()
            df_competencies = df_competencies[df_competencies['Competency Name'] != '']
            
            st.success(f"✅ File uploaded and cleaned! Found {len(df_competencies)} valid competencies to process.")
            ...
            if st.button("🚀 Generate 5 SJTs per Competency", type="primary", use_container_width=True):
                if df_competencies.empty:
                    st.error("No valid competencies found in the uploaded file after cleaning. Please check your file and try again.")
                else:
                    api_key = get_api_key()
                    ...
                    with st.spinner("Athena is crafting behavioral dilemmas... This may take a few minutes."):
                        for i, row in df_competencies.iterrows():
                            ...
                            if len(pl1_indicators) < 2:
                                st.warning(f"Skipping '{competency_name}' as it has fewer than 2 indicators after parsing.")
                                continue
